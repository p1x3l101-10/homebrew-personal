cask "docker" do
  ...
  on_intel do
    binary "Docker.app/Contents/Resources/bin/com.docker.hyperkit",
           target: "#{HOMEBREW_PREFIX}/local/bin/hyperkit"
  end
  ...
  binary "#{appdir}/Docker.app/Contents/Resources/bin/docker-credential-ecr-login",
         target: "#{HOMEBREW_PREFIX}/local/bin/docker-credential-ecr-login"
  binary "#{appdir}/Docker.app/Contents/Resources/bin/docker-credential-osxkeychain",
         target: "#{HOMEBREW_PREFIX}/local/bin/docker-credential-osxkeychain"
  binary "#{appdir}/Docker.app/Contents/Resources/bin/hub-tool",
         target: "#{HOMEBREW_PREFIX}/local/bin/hub-tool"
  binary "#{appdir}/Docker.app/Contents/Resources/bin/kubectl",
         target: "#{HOMEBREW_PREFIX}/local/bin/kubectl.docker"
  binary "#{appdir}/Docker.app/Contents/Resources/cli-plugins/docker-compose",
         target: "#{HOMEBREW_PREFIX}/local/cli-plugins/docker-compose"
  ...
  postflight do
    kubectl_target = Pathname("#{HOMEBREW_PREFIX}/local/bin/kubectl")
  ...
  zap trash: [
        "#{HOMEBREW_PREFIX}/local/bin/docker-compose.backup",
        "#{HOMEBREW_PREFIX}/local/bin/docker.backup",
        "~/.docker",
